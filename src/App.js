...
class Trails extends  React.Component {
  filterTrails(trails) {
    ...
    if (filter.weather === 1)
      this.filterByWeather(trails);
    if (filter.condition === 1)
      this.filterByCondition(trails);
  }

  filterByWeather(trails) {
  }

  filterByCondition(trails) {
    let time = new Date();
  ...
  render() {
    ...
    this.filterTrails(trails);

    return (
      <div className="trails">
        {/*<Trails_find getTrails={getTrails} />*/}
        <div className='trails-title'>Hot Trails</div>
        <Trails_list trails={trails} />
        <Trails_map trails={trails} id='trails-map' options={{center: { lat: 45.5051, lng: -122.6750 }, zoom: 13, mapTypeId: 'terrain'}} />
      </div>
    );
  }
}
...
function App() {
  return (
    <div>
      <Welcome title={(<Trails_find getTrails={getTrails} />)} />
      <Trails />
      <Footer />
    </div>
  );
